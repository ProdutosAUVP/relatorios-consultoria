...
.pf-topo .rt-vaga,.pf-topo .rt-img{margin:0;align-self:end}
...
.pf-rodape{padding:4mm 0 6mm;border-top:1px solid var(--line);
  display:grid;grid-template-columns:1.2fr 1fr;gap:4mm 8mm;align-items:start}
.pf-rodape h3{margin:0 0 2mm;font-size:6.2pt;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:var(--ink-2)}
/* O e-mail é o item mais largo da página e não tem onde quebrar; a coluna foi
   dimensionada para o maior deles caber numa linha, e o `anywhere` é a rede
   para um endereço ainda mais longo não vazar a margem. */
.pf-rodape .dl{font-size:8pt;gap:1mm 4mm}
.pf-rodape .dl dd{min-width:0;overflow-wrap:anywhere}
...
/* ---------- páginas do plano ----------
   O ritmo é o mesmo com fundo claro ou escuro: `.plano` cuida do respiro e da
   escala, `.dark` só da cor. As grelhas que seguem um bloco de texto herdam o
   mesmo vão do h2, e a escala sobe um pouco porque estas páginas têm menos
...
.page.plano .cols2{gap:12mm}
.page.plano .dl{font-size:9pt;gap:3mm 6mm}

/* ---------- texto corrido das páginas de leitura ----------
   A apresentação do consultor é, na maior parte, texto para ler do começo ao
   fim, e não lista para bater o olho. Esta é a escala dele: corpo um pouco
   maior que o das listas, entrelinha larga, e medida curta o bastante para o
   olho achar o começo da linha seguinte sem se perder. Dentro de uma coluna a
   medida já está dada pela grelha, e o teto sai do caminho. */
.page.plano .corrido{font-size:11pt;line-height:1.75;max-width:152mm}
.page.plano .corrido p{margin:0 0 5mm}
